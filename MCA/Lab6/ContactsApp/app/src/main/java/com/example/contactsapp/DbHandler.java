...
//            user.put("id",cursor.getLong(cursor.getColumnIndex("_id")));
    ...
    public ArrayList<HashMap<String, String>> GetID(String name){
        SQLiteDatabase db = this.getWritableDatabase();
        ArrayList<HashMap<String, String>> userList = new ArrayList<>();
        Cursor cursor = db.query(TABLE_Users, new String[] { KEY_ID,
                        KEY_NAME, KEY_DESG, KEY_LOC }, KEY_NAME + "=?",
    ...
    public boolean DeleteName(String name){
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(TABLE_Users, KEY_NAME+" = ?", new String[]{name})>0;
    ...
    public int UpdateUser(String name, String location, String designation ){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cVals = new ContentValues();
        cVals.put(KEY_LOC, location);
        cVals.put(KEY_DESG, designation);
        int count = db.update(TABLE_Users, cVals, KEY_NAME+" = ?",new String[]{name});
        return  count;
    }
}